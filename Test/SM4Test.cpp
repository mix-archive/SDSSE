...
int main() {
  ...
  auto *plaintext = (unsigned char *)"The test segmentation for SM4_CTR mode";

  unsigned char ciphertext[128];

  int ciphertext_len =
      sm4_encrypt(plaintext, strlen((char *)plaintext), key, iv, ciphertext);

  std::cout << "Input size:" << ciphertext_len << std::endl;

  unsigned char recover[128];
  int plaintext_len = sm4_decrypt(ciphertext, ciphertext_len, key, iv, recover);
