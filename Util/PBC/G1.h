...
class G1 : public G {
public:
  static G1 pow2(const Pairing &e, const G1 &base1, const Zr &exp1,
                 const G1 &base2, const Zr &exp2);
  static G1 pow3(const Pairing &e, const G1 &base1, const Zr &exp1,
                 const G1 &base2, const Zr &exp2, const G1 &base3,
                 const Zr &exp3);
  ...
  const G1 operator*(const G1 &rhs) const { return G1(*this) *= rhs; }
  const G1 operator/(const G1 &rhs) const { return G1(*this) /= rhs; }

  const G1 operator^(const Zr &exp) const { return G1(*this) ^= exp; }

  bool operator==(const G1 &rhs) const { return G::operator==(rhs); }
