...
Zr::Zr(const Pairing &e, const unsigned char *data, unsigned short len,
       unsigned short base) {
  elementPresent = e.isPairingPresent();
  if (elementPresent) {
    element_init_Zr(r, *(pairing_t *)&e.getPairing());
...
base){ if(len > 0){ nullify(); elementPresent = data[0];++data;--len;
        if(elementPresent){
          if(e.isPairingPresent()){
                element_init_Zr(r, *(pairing_t*)&e.getPairing());
                //if (compressed)
...
void Zr::dump(FILE *f, const char *label, unsigned short base) const {
  if (label)
    fprintf(f, "%s: ", label);
  // fprintf(f, "[ ");
  if (elementPresent)
