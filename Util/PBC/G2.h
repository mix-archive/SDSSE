...
class G2 : public G {
public:
  static G2 pow2(const Pairing &e, const G2 &base1, const Zr &exp1,
                 const G2 &base2, const Zr &exp2);
  static G2 pow3(const Pairing &e, const G2 &base1, const Zr &exp1,
                 const G2 &base2, const Zr &exp2, const G2 &base3,
                 const Zr &exp3);
  ...
  G2(const Pairing &e, const unsigned char *data, unsigned short len,
  ...
  const G2 operator*(const G2 &rhs) const { return G2(*this) *= rhs; }
  const G2 operator/(const G2 &rhs) const { return G2(*this) /= rhs; }

  const G2 operator^(const Zr &exp) const { return G2(*this) ^= exp; }

  bool operator==(const G2 &rhs) const { return G::operator==(rhs); }
