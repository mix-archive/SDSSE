...
class G { // Group
public:
  static void pow2(G &gout, const G &base1, const Zr &exp1, const G &base2,
                   const Zr &exp2);
  static void pow3(G &gout, const G &base1, const Zr &exp1, const G &base2,
  ...
  //				  bool compressed = false, unsigned short base =
  // 16);
...
protected:
  ...
  G(const Pairing &e) { elementPresent = e.isPairingPresent(); }
...
public:
  ...
  const T operator^(const Zr &exp) const {
    ...
    element_pp_pow_zn(*(element_t *)&res.getElement(),
                      *(element_t *)&exp.getElement(), *(element_pp_t *)&g_pp);
    return res;
  }
