...
Pairing::Pairing(const string &buf) { init(buf); }
  ...
  if (pairingPresent)
    if (p.isElementPresent() && q.isElementPresent()) {
      ...
      pairing_apply(gt, *(element_t *)&p.getElement(),
                    *(element_t *)&q.getElement(), *(pairing_t *)&e);
      GT ans;
      ans.setElement(gt);
      element_clear(gt);
      return ans;
...
// Symmetric Pairings
const GT Pairing::operator()(const G1 &p, const G1 &q) const {
  if (pairingPresent)
    if (p.isElementPresent() && q.isElementPresent())
      if (isSymmetric()) {
        element_t gt;
...
const GT Pairing::operator()(const G2 &p, const G2 &q) const {
  if (pairingPresent)
    if (p.isElementPresent() && q.isElementPresent())
      if (isSymmetric()) {
        element_t gt;
...
size_t Pairing::getElementSize(PairingElementType type, bool compressed) const {
  if (pairingPresent) {
    switch (type) {
    case Type_G1:
      if (compressed)
...
generateTypeAPairingParam(const FILE *stream,
                                                  unsigned short rbits,unsigned
short qbits){ pbc_param_t p;
  //a_param_init(p);
...
generateTypeGPairingParam(const FILE *stream, bool MNTorFreeman,
                                                  unsigned int discriminant,
unsigned int bitlimit){ g_param_t p; cm_info_t cm; darray_t L; size_t
darraySize; unsigned int randomIndex;
