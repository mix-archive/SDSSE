...
GT GT::pow2(const Pairing &e, const GT &base1, const Zr &exp1, const GT &base2,
            const Zr &exp2) {
  GT gout(e, true);
...
GT GT::pow3(const Pairing &e, const GT &base1, const Zr &exp1, const GT &base2,
            const Zr &exp2, const GT &base3, const Zr &exp3) {
  GT gout(e, true);
...
GT::GT(const Pairing &e, const unsigned char *data, unsigned short len,
       unsigned short base)
    : G(e) {
  if (elementPresent) {
    element_init_GT(g, *(pairing_t *)&e.getPairing());
...
GT::GT(const Pairing &e, const void *data, unsigned short len) : G(e) {
  if (elementPresent) {
    element_init_GT(g, *(pairing_t *)&e.getPairing());
