...
void SSEClientHandler::update(OP op, const string& keyword, int ind, uint8_t* content, size_t content_len) {
    // compute the tag
    ...
    if(op == INS) {
        ...
        for(long index : indexes) {
            ...
            aes_encrypt(content, content_len,
                        derived_key, encrypted_id,
                        encrypted_id + AES_BLOCK_SIZE);
            // save the encrypted id in the list
            ciphertext_list.emplace_back((char*) encrypted_id, AES_BLOCK_SIZE + content_len);
        }
