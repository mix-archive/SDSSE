...
Zr VSDSSECQServer::Fp(uint8_t *input, size_t input_size, uint8_t *key) {
...
VSDSSECQServer::VSDSSECQServer(Pairing *e) {
    this->e = e;
    tags.clear();
    tmap.clear();
    st_t.clear();
...
void VSDSSECQServer::add_entries_in_TMap(const string& label, const string& tag, const string& st, vector<string>& ciphertext_list) {
    tags[label] = tag;
    st_t[label] = st;
    tmap[label] = move(ciphertext_list);
}

void VSDSSECQServer::add_entries_in_XMap(const string& label, const string& tag, const string& st, vector<string>& ciphertext_list) {
    tags[label] = tag;
    st_x[label] = st;
