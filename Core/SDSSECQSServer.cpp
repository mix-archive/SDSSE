...
Zr SDSSECQSServer::Fp(uint8_t *input, size_t input_size, uint8_t *key) {
...
void SDSSECQSServer::compute_leaf_keys(unordered_map<long, uint8_t*>& keys, const vector<GGMNode>& node_list, int level) {
...
SDSSECQSServer::SDSSECQSServer(Pairing *e) {
    this->e = e;
    tags.clear();
    tmap.clear();
    st_t.clear();
...
void SDSSECQSServer::add_entries_in_TMap(const string& label, const string& tag, const string& st, vector<string>& ciphertext_list) {
    tags[label] = tag;
    st_t[label] = st;
    tmap[label] = move(ciphertext_list);
}

void SDSSECQSServer::add_entries_in_XMap(const string& label, const string& tag, const string& st, vector<string>& ciphertext_list) {
    tags[label] = tag;
    st_x[label] = st;
    xmap[label] = move(ciphertext_list);
}

vector<uint8_t*> SDSSECQSServer::search(int search_count, int level, int xterm_num, uint8_t *K_X,
    ...
    BloomFilter<128, XTAG_SIZE, XSET_HASH> wxset;
