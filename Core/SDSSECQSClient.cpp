...
Zr SDSSECQSClient::Hp(uint8_t *input, size_t input_size) {
    uint8_t PRF[DIGEST_SIZE];
...
vector<GGMNode> SDSSECQSClient::rev_key_generation(BloomFilter<DIGEST_SIZE, GGM_SIZE, HASH_SIZE>* deletion_map, uint8_t *key) {
...
vector<int> SDSSECQSClient::search(int count, ...) {
    ...
    vector<int> count_xs;
    // xtoken list
    vector<Zr> xt_list;                             // token list for masking
    ...
    // generate a demasking token xt = Fp(xterm, K_x) * Fp(sterm, K_z)
    xt_list.reserve(xterms.size());
    for (const string& xterm : xterms) {
        xt_list.emplace_back(Fp((uint8_t*) xterm.c_str(), xterm.size(), K_x)
            * Fp((uint8_t*) sterm.c_str(), sterm.size(), K_z));
    }
    va_end(keyword_list);
    ...
    vector<uint8_t*> encrypted_res_list =
            server->search(sterm_search_count, tree->get_level(), xterms.size(),
                           ...
                           K_wxs,
                           state_xs,count_xs,
                           X_revoked_key_lists, xt_list, xtoken_list,
                           token_X_str_list);
    // decrypt e locally
